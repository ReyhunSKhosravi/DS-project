...
void SHORT :: shortest_way(const vector<Vertex*>& vertices, const string& sourceName, const string& destinationName) {
    ...
    void SHORT::lowestcost (const vector<Vertex*>& vertices, const string& sourceName, const string& destinationName)  {
    Vertex* origin = nullptr;
    int weight;
    Vertex* destination = nullptr;
    for (Vertex* vertex : vertices) {
    ...
    while (!pq.empty()) {
        ...
        for (auto& neighbor : u->neighbors) { 
            Vertex* v = neighbor.first; // maghsad
            if(destination->color == origin->color && destination.TransportTypes.Subway == origin.TransportTypes.Subway)
            {
                 weight = 0;//vazne yal
            }
            else
            {
                weight = neighbor.second.first;
            }
            if (dist + weight < distance[v]) {
                distance[v] = dist + weight;
                pq.push({distance[v], v});
    ...
    void SHORT :: shorted_time(const vector<Vertex*>& vertices, const string& sourceName, const string& destinationName) {
        Vertex* origin = nullptr;
        int weight;
        Vertex* destination = nullptr;
        for (Vertex* vertex : vertices) {
        ...
        while (!pq.empty()) {
            ...
            for (auto& neighbor : u->neighbors) { 
                Vertex* v = neighbor.first; // maghsad
                if(current->neighbors[parent[currant]].second == "Subway")
                {
                 weight =( neighbor.second.first) * 1 ;//vazne yal
                }
                else if(current->neighbors[parent[currant]].second == "Taxi")
                {
                 weight =( neighbor.second.first) * 2 ;//vazne yal
                }
                else if(current->neighbors[parent[currant]].second == "Bus")
                {
                 weight =( neighbor.second.first) * 4 ;//vazne yal
                }
                else if()
                {
