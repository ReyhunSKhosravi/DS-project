...
vector<Vertex*> readGraphFromFile(const string& filename) {
    ...
    while (getline(file, line)) {
        ...
        double cost;
        int  time;
        if (iss >> originName >> destName >> weight >> transportTypeStr>>colour>>cost>>time) {
            ...
            vertexMap[originName]->neighbors[vertexMap[destName]] = make_pair(cost, transportType);
        }
    }
    file.close();
    return vertices;
...
void dijkstra(const vector<Vertex*>& vertices, const string& sourceName, const string& destinationName)
 {
    ...
    if (distance[destination] == numeric_limits<int>::max()) {
        ...
        cout << ", Distance: " << distance[destination] <<"  sum :" << sum << endl;
    }
}

int main() {
    try {
        string filename = "FinalCsvInput.txt";
        vector<Vertex*> vertices = readGraphFromFile(filename);
        string originName = "Shahrak-e_Shari'ati";
        string destinationName = "Kouhsar";
