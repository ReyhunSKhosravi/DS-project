...
int main(){
    ...
    while(getline(file, line)){
        ...
        int weight;
        j = 0;
        //weight = stoi(weightsBus);
        weight = stoi(weightAsli);
        while(getline(file2, line2)){
            if(j >= i){
                ...
                if((transportTypeStrBus == "BUS" && transportTypeStrAsli == "BUS") || (transportTypeStrBus == "SUBWAY" && transportTypeStrAsli == "SUBWAY")){
                    ...
                    if((transportTypeStrBus == "TAXI" && transportTypeStrAsli == "TAXI") ){
                    cout << line2 << endl;
                    if((originNameAsli != originNameBus) && (colorOfLineAsli == colorOfLineBus))
                        {
                            if(transportTypeStrBus == "TAXI"){
                                busInFile << originNameAsli << "\t" <<originNameBus << "\t" << (weight * 6000) * 1.5 << "\t" << transportTypeStrBus << "\t" << colorOfLineBus << "\n";
                                busInFile << originNameBus  << "\t" << originNameAsli << "\t" << (weight * 6000) * 1.5 << "\t" << transportTypeStrBus << "\t" << colorOfLineBus << "\n";
