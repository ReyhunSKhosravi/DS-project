...
void dijkstra(const vector<Vertex*>& vertices, const string& sourceName, const string& destinationName) {
    ...
    if (distance[destination] == numeric_limits<int>::max()) {
        ...
        for (auto it = path.rbegin(); it != path.rend(); ++it) {
            ...
            switch (transportTypes.back()) {
                case TransportType::Subway:
                {
                    ...
                    distance[destination]= distance[destination] + 8;
                }
                    break;
                case TransportType::Bus:
                {
                    cout << "Bus";
                    distance[destination] = distance[destination]+15;
                    break;
                }
                case TransportType::Taxi:
                {
                    cout << "Taxi";
                    distance[destination] = distance[destination]+5;
                }
                    break;
            }
            cout << ")";
        ...
        cout << ", Time Cost: " << distance[destination] << endl;
    }
}

int main() {
    try {
        string filename = "busInput.txt";
        vector<Vertex*> vertices = readGraphFromFile(filename);
        string originName = "Darvazeh_Dowlat";
        string destinationName = "Ferdowsi";
